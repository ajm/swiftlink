...
unsigned SimwalkDescentGraph::get_random_person() {
	return get_random(ped->num_members());
}

unsigned SimwalkDescentGraph::get_random_nonleaf() {
...
unsigned SimwalkDescentGraph::get_random_nonfounder() {
	return get_random(ped->num_members() - ped->num_founders()) + ped->num_founders();
}

unsigned SimwalkDescentGraph::get_random_locus() {
	return get_random(ped->num_markers());
}

enum parentage SimwalkDescentGraph::get_random_parent() {
...
// for each child, 
//		if you inherit from maternal change to paternal + vice versa		
void SimwalkDescentGraph::transition_t1(Person* p, unsigned locus) {
