...
curandState* GPUWrapper::gpu_init_random_curand() {
    ...
    int thread_num = num_threads_per_block() * num_blocks();
    fstream randfile;
    
    host_seeds = (long int*) malloc(sizeof(*host_seeds) * thread_num);
    if(! host_seeds) {
        fprintf(stderr, "error: %s (%s:%d)\n", strerror(errno), __FILE__, __LINE__);
        abort();
    }
    ...
    CUDA_CALLANDTEST(cudaMalloc((void**) &dev_seeds,   sizeof(*dev_seeds) * thread_num));
    CUDA_CALLANDTEST(cudaMemcpy(dev_seeds, host_seeds, sizeof(*dev_seeds) * thread_num, cudaMemcpyHostToDevice));
    
    CUDA_CALLANDTEST(cudaMalloc((void**) &dev_rng_state, sizeof(*dev_rng_state) * thread_num));
    
    
    run_gpu_curand_init_kernel(num_blocks(), num_threads_per_block(), dev_rng_state, dev_seeds);
...
tinymt32_status_t* GPUWrapper::gpu_init_random_tinymt() {
    ...
    params = (uint32_t*) malloc(sizeof(*params) * 3 * thread_num);
    if(! params) {
        fprintf(stderr, "error: %s (%s:%d)\n", strerror(errno), __FILE__, __LINE__);
        abort();
    }
    ...
    if(tinymt32_set_params(TINYMT_PARAM_FILENAME, params, thread_num) != 0) {
        fprintf(stderr, "error: could not read in parameters from %s\n", TINYMT_PARAM_FILENAME);
        abort();
    }
    
    seeds = (uint32_t*) malloc(sizeof(*seeds) * thread_num);
    if(! seeds) {
        fprintf(stderr, "error: %s (%s:%d)\n", strerror(errno), __FILE__, __LINE__);
        abort();
    }
    ...
    CUDA_CALLANDTEST(cudaMalloc((void**) &dparams, sizeof(*dparams) * 3 * thread_num));
    CUDA_CALLANDTEST(cudaMemcpy(dparams, params, sizeof(*dparams) * 3 * thread_num, cudaMemcpyHostToDevice));
    
    CUDA_CALLANDTEST(cudaMalloc((void**) &dseeds, sizeof(*dseeds) * thread_num));
    CUDA_CALLANDTEST(cudaMemcpy(dseeds, seeds, sizeof(*dseeds) * thread_num, cudaMemcpyHostToDevice));
    
    
    CUDA_CALLANDTEST(cudaMalloc((void**) &dev_rng_state, sizeof(*dev_rng_state) * thread_num));
    
    
    run_gpu_tinymt_init_kernel(num_blocks(), num_threads_per_block(), dev_rng_state, dparams, dseeds);
